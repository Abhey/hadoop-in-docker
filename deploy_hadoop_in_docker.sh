...
if [[ $? == 0 ]]; then
    ...
    docker run -itd --network hadoop-net -p 9870:9870 -p 8088:8088 -p 19888:19888 --name hadoop-master -h hadoop-master hadoop-in-docker:latest
...
docker run -itd --network hadoop-net --name hadoop-slave-1 -h hadoop-slave-1 hadoop-in-docker:latest
...
docker run -itd --network hadoop-net --name hadoop-slave-2 -h hadoop-slave-2 hadoop-in-docker:latest
